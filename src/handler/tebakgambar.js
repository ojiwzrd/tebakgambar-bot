...
*/
import dbUsers from '../utils/lowdb.js'
import dbGames from '../utils/gameDb.js'
...
// Function Answer Tebakgambar
export async function Tebakgambar(ctx) {
  const chatId = ctx.chat.id
  const userId = ctx.from.id

  await dbGames.read()
  dbGames.data.tebakgambar ||= []

  const game = dbGames.data.tebakgambar.find(g => g.chatId === chatId)
  if (!game) return

  if (!ctx.message.reply_to_message || ctx.message.reply_to_message.message_id !== game.messageId) return

  const jawabanUser = ctx.message.text?.toLowerCase().trim()
  if (!jawabanUser) return

  if (jawabanUser === game.jawaban) {
    dbGames.data.tebakgambar = dbGames.data.tebakgambar.filter(g => g.chatId !== chatId)
    await dbGames.write()

    await dbUsers.read()
    dbUsers.data.users ||= []

    let user = dbUsers.data.users.find(u => u.id === userId)
    if (!user) {
      user = {
        id: userId,
    ...
    user.game_data.tebakgambar ??= { score: 0 }
    const randomPoint = Math.floor(Math.random() * 10) + 1
    user.game_data.tebakgambar.score += randomPoint

    await dbUsers.write()
    // Tags users winner
    const username = ctx.from.username ? `@${ctx.from.username}` : ctx.from.first_name

    const correctAnswerText = interpolate(ctx.lang.__('correct_answer'), {})
    const congratsForText = interpolate(ctx.lang.__('congrats_for'), { username })
    const pointsText = interpolate(ctx.lang.__('points'), {})

    const text =
      `${correctAnswerText}\n\n` +
      `${congratsForText}\n` +
      `💰 Mendapatkan +${randomPoint} ${pointsText}\n`
    // Finnal Winners
    return ctx.reply(text, {
      reply_to_message_id: ctx.message.message_id,
      reply_markup: Markup.keyboard([[interpolate(ctx.lang.__('try_again_button'), {})]])
        .resize()
        .oneTime()
    })
  } else {
    return ctx.reply(ctx.lang.__('wrong_answer'), {
      reply_to_message_id: ctx.message.message_id
    })
  }
}