...
export function UcapanWaktu() {
  ...
  const jamFormatted = `${hour.toString().padStart(2, '0')}:${minute.toString().padStart(2, '0')}` // Format 24 Hour
  const tanggalFormatted = `${day}/${month}/${year}`

  let ucapan = ''

// Speech & Time
  if (hour >= 4 && hour < 6) ucapan = `🌅 Selamat dini hari`
  else if (hour >= 6 && hour < 10) ucapan = `☀️ Selamat pagi`
  else if (hour >= 10 && hour < 15) ucapan = `🌤️ Selamat siang`
  else if (hour >= 15 && hour < 18) ucapan = `🌥️ Selamat sore`
  else if (hour >= 18 && hour < 21) ucapan = `🌇 Selamat petang`
  else if (hour >= 21 && hour < 24) ucapan = `🌙 Selamat malam`
  else if (hour >= 0 && hour < 4) ucapan = `🌌 Larut malam`

// Final
return `${ucapan} | ${tanggalFormatted} | Jam ${jamFormatted}`
}